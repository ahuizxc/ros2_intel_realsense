...
BaseRealSenseNode::BaseRealSenseNode(rclcpp::Node node,
    ...
    _dev(dev), _pnh(node), _json_file_path(""),
    ...
    _intialize_time_base(false)
{
    ...
    _unit_step_size[DEPTH] = sizeof(uint16_t); // sensor_msgs::msg::ImagePtr row step size
    ...
    _unit_step_size[INFRA1] = sizeof(uint8_t); // sensor_msgs::msg::ImagePtr row step size
    ...
    _unit_step_size[INFRA2] = sizeof(uint8_t); // sensor_msgs::msg::ImagePtr row step size
    ...
    _unit_step_size[COLOR] = 3; // sensor_msgs::msg::ImagePtr row step size
    ...
    _unit_step_size[FISHEYE] = sizeof(uint8_t); // sensor_msgs::msg::ImagePtr row step size
    ...
    _unit_step_size[GYRO] = sizeof(uint8_t); // sensor_msgs::msg::ImagePtr row step size
    ...
    _unit_step_size[ACCEL] = sizeof(uint8_t); // sensor_msgs::msg::ImagePtr row step size
...
void BaseRealSenseNode::toggleSensors(bool enabled)
{
    for (auto it=_sensors.begin(); it != _sensors.end(); it++)
    {
        ...
        {
            RCUTILS_LOG_DEBUG("toggleSensors: " << ex.what());
        }
    }
}
...
{
    RCUTILS_LOG_INFO("Dynamic reconfig parameters is not implemented in the base node.");
}
...
void BaseRealSenseNode::getParameters()
{
    ...
    _pnh->get_parameter_or("align_depth", _align_depth, ALIGN_DEPTH);
    _pnh->get_parameter_or("enable_pointcloud", _pointcloud, POINTCLOUD);
    std::string pc_texture_stream("");
    int pc_texture_idx;
    _pnh->get_parameter_or("pointcloud_texture_stream", pc_texture_stream, std::string("RS2_STREAM_COLOR"));
    _pnh->get_parameter_or("pointcloud_texture_index", pc_texture_idx, 0);
    _pointcloud_texture = stream_index_pair{rs2_string_to_stream(pc_texture_stream), pc_texture_idx};

    _pnh->get_parameter_or("filters", _filters_str, DEFAULT_FILTERS);
    _pointcloud |= (_filters_str.find("pointcloud") != std::string::npos);

    _pnh->get_parameter_or("enable_sync", _sync_frames, SYNC_FRAMES);
    if (_pointcloud || _align_depth || _filters_str.size() > 0)
        _sync_frames = true;

    _pnh->get_parameter_or("json_file_path", _json_file_path, std::string(""));

    _pnh->get_parameter_or("depth_width", _width[DEPTH], DEPTH_WIDTH);
    _pnh->get_parameter_or("depth_height", _height[DEPTH], DEPTH_HEIGHT);
    _pnh->get_parameter_or("depth_fps", _fps[DEPTH], DEPTH_FPS);
    _pnh->get_parameter_or("enable_depth", _enable[DEPTH], ENABLE_DEPTH);

    _pnh->get_parameter_or("infra1_width", _width[INFRA1], INFRA1_WIDTH);
    _pnh->get_parameter_or("infra1_height", _height[INFRA1], INFRA1_HEIGHT);
    _pnh->get_parameter_or("infra1_fps", _fps[INFRA1], INFRA1_FPS);
    _pnh->get_parameter_or("enable_infra1", _enable[INFRA1], ENABLE_INFRA1);

    _pnh->get_parameter_or("infra2_width", _width[INFRA2], INFRA2_WIDTH);
    _pnh->get_parameter_or("infra2_height", _height[INFRA2], INFRA2_HEIGHT);
    _pnh->get_parameter_or("infra2_fps", _fps[INFRA2], INFRA2_FPS);
    _pnh->get_parameter_or("enable_infra2", _enable[INFRA2], ENABLE_INFRA2);

    _pnh->get_parameter_or("color_width", _width[COLOR], COLOR_WIDTH);
    _pnh->get_parameter_or("color_height", _height[COLOR], COLOR_HEIGHT);
    _pnh->get_parameter_or("color_fps", _fps[COLOR], COLOR_FPS);
    _pnh->get_parameter_or("enable_color", _enable[COLOR], ENABLE_COLOR);

    _pnh->get_parameter_or("fisheye_width", _width[FISHEYE], FISHEYE_WIDTH);
    _pnh->get_parameter_or("fisheye_height", _height[FISHEYE], FISHEYE_HEIGHT);
    _pnh->get_parameter_or("fisheye_fps", _fps[FISHEYE], FISHEYE_FPS);
    _pnh->get_parameter_or("enable_fisheye", _enable[FISHEYE], ENABLE_FISHEYE);

    _pnh->get_parameter_or("gyro_fps", _fps[GYRO], GYRO_FPS);
    _pnh->get_parameter_or("accel_fps", _fps[ACCEL], ACCEL_FPS);
    _pnh->get_parameter_or("enable_imu", _enable[GYRO], ENABLE_IMU);
    _pnh->get_parameter_or("enable_imu", _enable[ACCEL], ENABLE_IMU);

    _pnh->get_parameter_or("base_frame_id", _base_frame_id, DEFAULT_BASE_FRAME_ID);
    _pnh->get_parameter_or("depth_frame_id", _frame_id[DEPTH], DEFAULT_DEPTH_FRAME_ID);
    _pnh->get_parameter_or("infra1_frame_id", _frame_id[INFRA1], DEFAULT_INFRA1_FRAME_ID);
    _pnh->get_parameter_or("infra2_frame_id", _frame_id[INFRA2], DEFAULT_INFRA2_FRAME_ID);
    _pnh->get_parameter_or("color_frame_id", _frame_id[COLOR], DEFAULT_COLOR_FRAME_ID);
    _pnh->get_parameter_or("fisheye_frame_id", _frame_id[FISHEYE], DEFAULT_FISHEYE_FRAME_ID);
    _pnh->get_parameter_or("imu_gyro_frame_id", _frame_id[GYRO], DEFAULT_IMU_FRAME_ID);
    _pnh->get_parameter_or("imu_accel_frame_id", _frame_id[ACCEL], DEFAULT_IMU_FRAME_ID);

    _pnh->get_parameter_or("depth_optical_frame_id", _optical_frame_id[DEPTH], DEFAULT_DEPTH_OPTICAL_FRAME_ID);
    _pnh->get_parameter_or("infra1_optical_frame_id", _optical_frame_id[INFRA1], DEFAULT_INFRA1_OPTICAL_FRAME_ID);
    _pnh->get_parameter_or("infra2_optical_frame_id", _optical_frame_id[INFRA2], DEFAULT_INFRA2_OPTICAL_FRAME_ID);
    _pnh->get_parameter_or("color_optical_frame_id", _optical_frame_id[COLOR], DEFAULT_COLOR_OPTICAL_FRAME_ID);
    _pnh->get_parameter_or("fisheye_optical_frame_id", _optical_frame_id[FISHEYE], DEFAULT_FISHEYE_OPTICAL_FRAME_ID);
    _pnh->get_parameter_or("gyro_optical_frame_id", _optical_frame_id[GYRO], DEFAULT_GYRO_OPTICAL_FRAME_ID);
    _pnh->get_parameter_or("accel_optical_frame_id", _optical_frame_id[ACCEL], DEFAULT_ACCEL_OPTICAL_FRAME_ID);

    _pnh->get_parameter_or("aligned_depth_to_color_frame_id",   _depth_aligned_frame_id[COLOR],   DEFAULT_ALIGNED_DEPTH_TO_COLOR_FRAME_ID);
    _pnh->get_parameter_or("aligned_depth_to_infra1_frame_id",  _depth_aligned_frame_id[INFRA1],  DEFAULT_ALIGNED_DEPTH_TO_INFRA1_FRAME_ID);
    _pnh->get_parameter_or("aligned_depth_to_infra2_frame_id",  _depth_aligned_frame_id[INFRA2],  DEFAULT_ALIGNED_DEPTH_TO_INFRA2_FRAME_ID);
    _pnh->get_parameter_or("aligned_depth_to_fisheye_frame_id", _depth_aligned_frame_id[FISHEYE], DEFAULT_ALIGNED_DEPTH_TO_FISHEYE_FRAME_ID);
}
...
void BaseRealSenseNode::setupDevice()
{
    ...
    try{
        if (!_json_file_path.empty())
        {
            if (_dev.is<rs400::advanced_mode>())
            {
                ...
                if (in.is_open())
                {
                    ...
                    RCUTILS_LOG_INFO("JSON file is loaded! (" << _json_file_path << ")");
                }
                else
                    RCUTILS_LOG_WARN("JSON file provided doesn't exist! (" << _json_file_path << ")");
            }
            else
                RCUTILS_LOG_WARN("Device does not support advanced settings!");
        }
        else
            RCUTILS_LOG_INFO("JSON file is not provided");

        RCUTILS_LOG_INFO("ROS Node Namespace: " << _namespace);

        auto camera_name = _dev.get_info(RS2_CAMERA_INFO_NAME);
        RCUTILS_LOG_INFO("Device Name: " << camera_name);

        RCUTILS_LOG_INFO("Device Serial No: " << _serial_no);

        auto fw_ver = _dev.get_info(RS2_CAMERA_INFO_FIRMWARE_VERSION);
        RCUTILS_LOG_INFO("Device FW version: " << fw_ver);

        auto pid = _dev.get_info(RS2_CAMERA_INFO_PRODUCT_ID);
        RCUTILS_LOG_INFO("Device Product ID: 0x" << pid);

        RCUTILS_LOG_INFO("Enable PointCloud: " << ((_pointcloud)?"On":"Off"));
        RCUTILS_LOG_INFO("Align Depth: " << ((_align_depth)?"On":"Off"));
        RCUTILS_LOG_INFO("Sync Mode: " << ((_sync_frames)?"On":"Off"));
        ...
        for(auto&& elem : dev_sensors)
        {
            ...
            {
                RCUTILS_LOG_ERROR("Module Name \"" << module_name << "\" isn't supported by LibRealSense! Terminating RealSense Node...");
                rclcpp::shutdown();
                exit(1);
            }
            RCUTILS_LOG_INFO(std::string(elem.get_info(RS2_CAMERA_INFO_NAME)) << " was found.");
        }
        ...
        for (auto& elem : streams)
        {
            for (auto& stream_index : elem)
            {
                ...
                {
                    RCUTILS_LOG_INFO("(" << rs2_stream_to_string(stream_index.first) << ", " << stream_index.second << ") sensor isn't supported by current device! -- Skipping...");
    ...
    {
        RCUTILS_LOG_ERROR("An exception has been thrown: " << ex.what());
        throw;
    }
    catch(...)
    {
...
void BaseRealSenseNode::setupPublishers()
{
    ...
    for (auto& stream : image_stream_types)
    {
        if (_enable[stream])
        {
            ...
            _image_publishers[stream] = {image_transport::create_publisher(_pnh.get(), image_raw.str()), frequency_diagnostics};
            _info_publisher[stream] = _pnh.create_publisher<sensor_msgs::msg::CameraInfo>(camera_info.str(), 1);
            ...
            if (_align_depth && (stream != DEPTH))
            {
                ...
                _depth_aligned_image_publishers[stream] = {image_transport::create_publisher(_pnh.get(), aligned_image_raw.str()), frequency_diagnostics};
                _depth_aligned_info_publisher[stream] = _pnh.create_publisher<sensor_msgs::msg::CameraInfo>(aligned_camera_info.str(), 1);
            }

            if (stream == DEPTH && _pointcloud)
            {
                _pointcloud_publisher = _pnh.create_publisher<sensor_msgs::msg::PointCloud2>("depth/color/points", 1);
            }
        }
    }
    ...
    if (_enable[FISHEYE] &&
        _enable[DEPTH])
    {
        _depth_to_other_extrinsics_publishers[FISHEYE] = _pnh.create_publisher<Extrinsics>("extrinsics/depth_to_fisheye", 1, true);
    }

    if (_enable[COLOR] &&
        _enable[DEPTH])
    {
        _depth_to_other_extrinsics_publishers[COLOR] = _pnh.create_publisher<Extrinsics>("extrinsics/depth_to_color", 1, true);
    }

    if (_enable[INFRA1] &&
        _enable[DEPTH])
    {
        _depth_to_other_extrinsics_publishers[INFRA1] = _pnh.create_publisher<Extrinsics>("extrinsics/depth_to_infra1", 1, true);
    }

    if (_enable[INFRA2] &&
        _enable[DEPTH])
    {
        _depth_to_other_extrinsics_publishers[INFRA2] = _pnh.create_publisher<Extrinsics>("extrinsics/depth_to_infra2", 1, true);
    }

    if (_enable[GYRO])
    {
        _imu_publishers[GYRO] = _pnh.create_publisher<sensor_msgs::msg::Imu>("gyro/sample", 100);
        _info_publisher[GYRO] = _pnh.create_publisher<IMUInfo>("gyro/imu_info", 1, true);
    }

    if (_enable[ACCEL])
    {
        _imu_publishers[ACCEL] = _pnh.create_publisher<sensor_msgs::msg::Imu>("accel/sample", 100);
        _info_publisher[ACCEL] = _pnh.create_publisher<IMUInfo>("accel/imu_info", 1, true);
    }
}
                                             ...
                                             rs2_stream stream_type, int stream_index)
{
    ...
    {
        RCUTILS_LOG_ERROR("Stream type is not supported! (" << stream_type << ", " << stream_index << ")");
    }
}
		...
		for (auto& elem : streams)
		{
			if (_enable[elem])
			{
				...
				for (auto& profile : profiles)
				{
					...
					RCUTILS_LOG_DEBUG("Sensor profile: " <<
						...
						video_profile.stream_index() == elem.second)
					{
						...
						RCUTILS_LOG_INFO(_stream_name[elem] << " stream is enabled - width: " << _width[elem] << ", height: " << _height[elem] << ", fps: " << _fps[elem]);
						break;
					}
				}
...
void BaseRealSenseNode::setupFilters()
{
    ...
    for (std::vector<std::string>::const_iterator s_iter=filters_str.begin(); s_iter!=filters_str.end(); s_iter++)
    {
        ...
        {
            RCUTILS_LOG_ERROR("Unknown Filter: " << (*s_iter));
            throw;
        }
    }
    if (use_disparity_filter)
    {
        RCUTILS_LOG_INFO("Add Filter: disparity");
        ...
        RCUTILS_LOG_INFO("Done Add Filter: disparity");
    }
    if (use_colorizer_filter)
    {
        RCUTILS_LOG_INFO("Add Filter: colorizer");
        ...
        _unit_step_size[DEPTH] = _unit_step_size[COLOR]; // sensor_msgs::msg::ImagePtr row step size
    ...
    if (_pointcloud)
    {
    	RCUTILS_LOG_INFO("Add Filter: pointcloud");
    ...
    RCUTILS_LOG_INFO("num_filters: %d", static_cast<int>(_filters.size()));
}
...
void BaseRealSenseNode::setupStreams()
{
    ...
    try{
        ...
        auto frame_callback = [this](rs2::frame frame)
        {
            try{
                ...
                if (false == _intialize_time_base)
                {
                    if (RS2_TIMESTAMP_DOMAIN_SYSTEM_TIME == frame.get_frame_timestamp_domain())
                        RCUTILS_LOG_WARN("Frame metadata isn't available! (frame_timestamp_domain = RS2_TIMESTAMP_DOMAIN_SYSTEM_TIME)");
                ...
                if (frame.is<rs2::frameset>())
                {
                    ...
                    RCUTILS_LOG_DEBUG("List of frameset before applying filters: size: %d", static_cast<int>(frameset.size()));
                    for (auto it = frameset.begin(); it != frameset.end(); ++it)
                    {
                        ...
                        RCUTILS_LOG_DEBUG("Frameset contain (%s, %d, %s %d) frame. frame_number: %llu ; frame_TS: %f ; ros_TS(NSec): %lu",
                    ...
                    RCUTILS_LOG_DEBUG("num_filters: %d", static_cast<int>(_filters.size()));
                    for (std::vector<NamedFilter>::const_iterator filter_it = _filters.begin(); filter_it != _filters.end(); filter_it++)
                    {
                        RCUTILS_LOG_DEBUG("Applying filter: %s", filter_it->_name.c_str());
                        frameset = filter_it->_filter->process(frameset);
                    }

                    RCUTILS_LOG_DEBUG("List of frameset after applying filters: size: %d", static_cast<int>(frameset.size()));
                    for (auto it = frameset.begin(); it != frameset.end(); ++it)
                    {
                        auto f = (*it);
                        auto stream_type = f.get_profile().stream_type();
                    ...
                    RCUTILS_LOG_DEBUG("END OF LIST");
                    RCUTILS_LOG_DEBUG("Remove streams with same type and index:");
                    ...
                    for (auto it = frames_to_publish.begin(); it != frames_to_publish.end(); ++it)
                    {
                        ...
                        RCUTILS_LOG_DEBUG("Frameset contain (%s, %d, %s) frame. frame_number: %llu ; frame_TS: %f ; ros_TS(NSec): %lu",
                        ...
                        if (f.is<rs2::points>())
                        {
                            ...
                            {
                                RCUTILS_LOG_DEBUG("Publish pointscloud");
                    ...
                    {
                        RCUTILS_LOG_DEBUG("publishAlignedDepthToOthers(...)");
                ...
                else
                {
                    ...
                    auto stream_index = frame.get_profile().stream_index();
                    updateIsFrameArrived(is_frame_arrived, stream_type, stream_index);
                    RCUTILS_LOG_DEBUG("Single video frame arrived (%s, %d). frame_number: %llu ; frame_TS: %f ; ros_TS(NSec): %lu",
            ...
            {
                RCUTILS_LOG_ERROR("An error has occurred during frame callback: " << ex.what());
            }
        }; // frame_callback
            ...
            accel_profile != _enabled_profiles.end())
        {
            ...
            sens.start([this](rs2::frame frame){
                    ...
                    0 != _imu_publishers[stream_index].getNumSubscribers())
                {
                    ...
                    RCUTILS_LOG_DEBUG("Publish %s stream", rs2_stream_to_string(frame.get_profile().stream_type()));
                }
            });

            if (_enable[GYRO])
            {
                RCUTILS_LOG_INFO(_stream_name[GYRO] << " stream is enabled - " << "fps: " << _fps[GYRO]);
            ...
            if (_enable[ACCEL])
            {
                RCUTILS_LOG_INFO(_stream_name[ACCEL] << " stream is enabled - " << "fps: " << _fps[ACCEL]);
...
void BaseRealSenseNode::updateStreamCalibData(const rs2::video_stream_profile& video_profile)
{
    ...
    {
        RCUTILS_LOG_ERROR("Given depth profile is not supported by current device!");
        rclcpp::shutdown();
        exit(1);
    }
...
void BaseRealSenseNode::publishPointCloud(rs2::points pc, const rclcpp::Time& t, const rs2::frameset& frameset)
{
    ...
    if (use_texture)
    {
        ...
        {
            RCUTILS_LOG_DEBUG("texture frame not found");
            return;
        }
...
void BaseRealSenseNode::publishFrame(rs2::frame f, const rclcpp::Time& t,
                                     ...
                                     const std::map<stream_index_pair, rclcpp::Publisher>& info_publishers,
...
void BaseD400Node::callback(base_d400_paramsConfig &config, uint32_t level)
{
    ...
    if (set_default_dynamic_reconfig_values == level)
    {
        for (int i = 1 ; i < base_depth_count ; ++i)
        {
            ...
            {
                RCUTILS_LOG_ERROR("Failed. Skip initialization of parameter " << (base_depth_param)i);
            }
        }
    }
    else
...
void BaseD400Node::setParam(base_d400_paramsConfig &config, base_depth_param param)
{
    ...
    case base_depth_output_trigger_enabled:
        RCUTILS_LOG_DEBUG("base_depth_output_trigger_enabled: " << config.base_depth_output_trigger_enabled);
    ...
    {
        RCUTILS_LOG_DEBUG("base_sensors_enabled: " << config.base_sensors_enabled);
        toggleSensors(config.base_sensors_enabled);
        break;
    }
    case base_JSON_file_path:
    {
        RCUTILS_LOG_DEBUG("base_JSON_file_path: " << config.base_JSON_file_path);
        auto adv_dev = _dev.as<rs400::advanced_mode>();
        if (!adv_dev)
        {
            RCUTILS_LOG_WARN("Device doesn't support Advanced Mode!");
            return;
        }
        if (!config.base_JSON_file_path.empty())
        {
            ...
            if (!in.is_open())
            {
                RCUTILS_LOG_WARN("JSON file provided doesn't exist!");
                return;
            }
